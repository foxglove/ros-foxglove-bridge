...
namespace impl {
namespace {
bool has_topic(const rclcpp::Node& node, const std::string& topic,
               std::vector<std::string>& types) {
  const std::map<std::string, std::vector<std::string>>& topics = node.get_topic_names_and_types();
  auto it = std::find_if(topics.begin(), topics.end(), [&topic](const auto& entry) {
...
bool wait_for_topic_and_type_nanoseconds(rclcpp::Node& node, const std::string& topic,
  ...
  do {
    ...
    node.wait_for_graph_change(event,
                               std::min(time_to_wait, std::chrono::nanoseconds(RCL_MS_TO_NS(100))));

...
BabelFishSubscription::BabelFishSubscription(
  ...
  if (rclcpp::detail::resolve_use_intra_process(options, *node_base)) {
    ...
    //    // First create a SubscriptionIntraProcess which will be given to the intra-process
    //    manager. auto context = node_base->get_context(); subscription_intra_process_ =
    //    std::make_shared<SubscriptionIntraProcessT>(
    //      callback,
    //      options.get_allocator(),
    //      context,
    //      this->get_topic_name(),  // important to get like this, as it has the fully-qualified
    //      name qos_profile, resolve_intra_process_buffer_type( options.intra_process_buffer_type,
    //      callback ));
    //    TRACEPOINT(
    ...
    //    uint64_t intra_process_subscription_id = ipm->add_subscription(
    //    subscription_intra_process_ ); this->setup_intra_process( intra_process_subscription_id,
    //    ipm );
  }
...
void BabelFishSubscription::return_serialized_message(
